...
export default function AdminNewsClient({ news: initialNews }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (editing) {
      ...
      setSaving(false)
      if (!error) setNews((prev) => prev.map((n) => (n.id === data.id ? data : n)))
      resetForm()
      return
    }
    const { data, error } = await supabase.from('news').insert(payload).select().single()
    setSaving(false)
    if (!error) setNews((prev) => [data, ...prev])
    resetForm()
  }
          ...
          {news.map((n) => (
            <li key={n.id} className="p-4 flex flex-wrap items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-2">
                <button type="button" onClick={() => startEdit(n)} className="text-sm text-slate-600 hover:underline">Düzenle</button>
                <button type="button" onClick={() => toggleActive(n)} className={`text-sm px-3 py-1 rounded-lg ${n.is_active ? 'bg-green-100 text-green-800' : 'bg-slate-200 text-slate-600'}`}>
